...
const Claimform = () => {
    ...
    useEffect(()=>{
        ...
        fetch(`https://16.171.7.98/question/singel/verify/data/01/${username}`)
        .then(res => res.json())
        .then(data => setVeri(data))

        const key = process.env.REACT_APP_API_KEY
        fetch(`https://16.171.7.98/get/accountdata01/by/${username}?key=${key}`)
        .then(res => res.json())
    ...
    const Post = (e) =>{
        ...
        axios.post("https://16.171.7.98/account/data/post/get",{username, upi,name})
        .then(res =>{
