...
const Play = () => {
    ...
    useEffect(()=>{
        ...
        fetch(`https://16.171.7.98/q/amount/data/all?key=${key}`)
            ...
            fetch(`https://16.171.7.98/question/singel/verify/data/01/sakhd/sjkh/dsf/dfsd/${username}`)
            .then(res => res.json())
            .then(data => setUpii1(data))

            fetch(`https://16.171.7.98/question/singel/verify/data/01/sakhd/${username}`)
            .then(res => res.json())
    ...
    const Post = (e) =>{
        e.preventDefault();
        axios.post("https://16.171.7.98/verify/account/key",{username})
            .then(res =>{
            if(res.data.Status === "OK"){
                alert("Play After money Credited to you'r Account")
            }else{
                axios.post('https://16.171.7.98/data/upi/post/to/db',{username, Qno})
                .then(res =>{
                    if(res.data.Status === "OK"){
